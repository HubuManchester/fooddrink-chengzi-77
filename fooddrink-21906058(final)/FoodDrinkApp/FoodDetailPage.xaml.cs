...
public partial class FoodDetailPage : ContentPage
{
    ...
    public FoodDetailPage(DatabaseService databaseService)
    {
        InitializeComponent();
    ...
    private void RenderItem()
    {
        ...
        // Load food image
        if (!string.IsNullOrEmpty(currentItem.ImagePath) && File.Exists(currentItem.ImagePath))
        {
            FoodImage.Source = ImageSource.FromFile(currentItem.ImagePath);
        }
        else
        {
            string localImageName = $"{currentItem.Name?.ToLower().Replace(" ", "_")}.png";
            FoodImage.Source = ImageSource.FromFile(localImageName);
        }

        // Update nutrition labels for the new UI
        if (ProteinLabel != null && CarbsLabel != null && FatLabel != null)
        {
